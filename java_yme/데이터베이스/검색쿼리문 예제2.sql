-- join 은 한 테이블로 원하는 모든 정보를 가져올수 없을 때 여러 테이블을 하나로 만들어서 가져와야 하는 경우에 사용
...
-- 교수번호 P2000135001 교수님의 강의를 들은 학생명단 
select st_name as '학생 이름' from course,class,student 
...
-- 2020135001 학생의 수강한 강의 제목을 확인을 join을 활용하여 작성
select course.co_st_num as '학번', class.cl_title as '강의명' from course 
	join class  on course.co_cl_code = class.cl_code
...
select pr_name from class 
	join professor on pr_num = cl_pr_num 
		where cl_title = '네트워크';
