-- select 속성 from 주문 where 조건(속성 = 값(문자열인경우 ' ' 필요) ;  => 기본 문법
-- 고객 명단 확인(고객정보 전체 확인)
-- select * from 고객;
-- 고객 명단에서 고객들의 등급만 확인
-- select 등급 form 고객;
-- [all | distict]를 생략하면 all이 기본값으로 적용
-- select문에서 테이블명 앞에 데이터베이스명을 안쓰면 현재 선택된 데이터베이스에 따라
-- 쿼리문이 실행되지 않을수도 있다.
-- select distinct 등급 from 고객;
-- 고객의 등급을 확인(고객아이디와 등급을 확인)
-- select 고객아이디,등급 from 고객;
-- 검색 결과이 속성 이름을 변경하는 키워드 as
...
-- 정렬은 order by 속성명1 [asc:오름차순 | desc: 내림차순], 속성명2[asc|desc}, .... 로 정렬할 수 있다
