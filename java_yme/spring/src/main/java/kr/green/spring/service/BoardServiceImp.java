...
public class BoardServiceImp implements BoardService {
	...
	public int deleteBoard(Integer num, MemberVO user) {
		...
		board.setValid("D");
		//첨부파일 삭제, 해당 게시글과 일치하는 첨부파일 정보들을 가져 옴
		//반복문으로 하나씩 삭제 처리(deleteFileVO)
	...
	public int updateRecommend(MemberVO user, int board, int state) {
		...
		//있으면
			...
			//기존 state 정보와 누른 버튼 정보를 비교하여 같으면 state를 0으로 수정하고 0을 리턴(추천 누르고 추천, 비추천 누르고 비추천/취소)
			if(state == rvo.getState()) {
				rvo.setState(0);
				boardDao.updateRecommend(rvo);
				return 0;
			} 
			//다르면 누른 버튼 정보로 수정하고 1을 리턴(추천 누르고 비추천, 비추천 누르고 추천)
			else {
				rvo.setState(state);
				boardDao.updateRecommend(rvo);
				return 1;
