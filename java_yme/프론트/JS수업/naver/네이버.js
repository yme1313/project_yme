...
var id;
  ...
  $('.menu-list input[type=checkbox]').click(function(){
    ...
    var href = $(this).attr('data-garget')
...
/* 
- 메뉴 설정을 눌렀을 떄 검은 메뉴 창에 메뉴를 선택할 수 있는 박스로 보여주도록 
  화면을 구성하는 함수로, 선택된 메뉴가 있으면 글자가 아니면 박스가 보여지고 최대 4개 보여진다 
*/
...
function setMenuBox(selArr, oriArr, selHref, oriHref){
  //선택된 메뉴가 있으면 선택된 메뉴를 화면에 출력하고, 선택된 메뉴가 없으면 
  //미리 지정된 메뉴를 출력하기 위해 arr를 상황에 따라 선정
  var arr = selArr.length == 0 ? oriArr : selArr;
  var hrefArr = selHref.length == 0 ? oriArr : selHref;
...
//roll-btn-box에 다음 버튼 누르면
$('.roll-btn-box .next-btn').click(function(){
  ...
  if(selected.next().length == 0){
    ...
    //애니메이션
    var ml = $('.roll-wrap-box').css('margin-left');
    ml = parseFloat(ml);
    var width = $('.roll-wrap-box .menu').first().width();
    ml = ml - width;
    $('.roll-wrap-box').animate({'margin-left':ml+'px'},300);
    selected.parent().next().children('.menu-item').first().addClass('selected');
    selected.removeClass('selected');
  }else{
    selected.removeClass('selected').next().addClass('selected');
  }
  var Btn = $('.menu-item').first().hasClass('selected');
  var Btn1 = $('.menu-item').last().hasClass('selected');
    if(Btn == false){
      $('.roll-btn-box .prev-btn').show();
    }
    if(Btn1 == true){
      $('.roll-btn-box .next-btn').hide();
    }
  $('.selected').click();
})
//roll-btn-box에 이전 버튼 누르면
$('.roll-btn-box .prev-btn').click(function(){
  var selected = $('.selected');
  ...
  var Btn1 = $('.menu-item').last().hasClass('selected');
    if(Btn == true){
      $('.roll-btn-box .prev-btn').hide();
    }
    if(Btn1 == false){
      $('.roll-btn-box .next-btn').show();
    }
  $('.selected').click();
})
//roll-btn-box에 메뉴를 직접 누르면
